...
const AddAttendance = () => {
  ...
  useEffect(() => {
    ...
    // checking exisiting attendances
    const checkMarkedIn = async (employeeNumber) => {
        const response = await axios.get(
          `http://localhost:8000/attendance/check/${employeeNumber}`
        );
    
        if(response.data.status){
            navigate("/ownAttendance");
        }
      };

    //   calling to checking existing attendace function
      checkMarkedIn(storedEmpNo);
  }, []);
  ...
  const handleRecordAttendance = async (e) => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:8000/attendance/add",
        {
          empNo,
